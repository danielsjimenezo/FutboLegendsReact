...
function MyTop20() {
  ...
  const [myReplaceIndex, setMyReplaceIndex] = useState(
    getReplaceIndex(myTopPlayers)
  );
  ...
  useEffect(() => {
    localStorage.setItem(
      "futboLegends-myTopPlayers",
      JSON.stringify(myTopPlayers)
    );
  }, [myTopPlayers]);
  ...
  const handleMySearch = (e) => {
    ...
    if (term.length >= 2) {
      ...
      // Show all players when search term is empty
      setMySearchResults(players.slice(0, 10));
    }
  };
  ...
  const handleSearchFocus = () => {
    ...
    // Show initial results when focused
    setMySearchResults(players.slice(0, 10));
  };
      ...
      <div className="section-header">
        ...
        <div className="search-container" ref={myResultsRef}>
          <div className="search-box">
            ...
            <input
              type="text"
              placeholder={`Search for #${myReplaceIndex + 1}`}
              value={mySearchTerm}
            ...
            <ul className="search-results">
              {mySearchResults.map((player) =>
                player?.name ? (
                  ...
                  <li key={player.id}>PLAYER HERE</li>
                )
              )}
            </ul>
          )}
